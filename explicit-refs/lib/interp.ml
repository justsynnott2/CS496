...
let rec eval_expr : expr -> exp_val ea_result = fun e ->
  ...
  (*| IsEqual (e1,e2) ->
    ...
    eval_expr e2 >>= fun n2 ->
      (match (n1, n2) with
      | (NumVal nv1, NumVal nv2) -> return @@ BoolVal (nv1 = nv2))
      | _ -> error "e1 and e2 are not both NumVals"
  | IsGT (e1, e2) -> 
    eval_expr e1 >>= fun n1 ->
    eval_expr e2 >>= fun n2 ->
      (match (n1, n2) with
  ...
  | Proj (e,id) ->
    ...
    fields_of_recordVal >>= fun r ->
      ...
      | Some (is_mutable, ev) -> 
      if is_mutable then
        int_of_refVal ev >>= fun rv ->
        Store.deref g_store rv
      else return ev
  ...
  | SetField (e1, id, e2) ->
    ...
    (match List.assoc_opt id r with
      | Some (is_mutable, ev1) -> 
        if is_mutable 
          then int_of_refVal ev1 >>= fun ref ->
          eval_expr e2 >>= fun v -> 
          Store.set_ref g_store ref v >>= fun _ ->
          return UnitVal

...
let is_subclass : string -> string -> class_env -> exp_val ea_result =
  ...
  match List.assoc_opt id2 class_env with
    ...
    | _ -> if id1 = id2 
      ...
      let rec get_superclass c_name =
      match List.assoc_opt c_name class_env with
        | Some (sc, _, _) -> 
        if sc = "" then return @@ BoolVal false
        else if sc = id2 then return @@ BoolVal true
        else get_superclass sc
        | None -> error ("is_subclass: class " ^ id1 ^ " not found!")
      in get_superclass id1
...
and   
    ...
    (match List.assoc_opt c_name !g_class_env with
     ...
     | Some (_super,fields,methods) -> 
       ...
       in (match List.assoc_opt "initialize_0" methods with
           | None -> return self
           | Some m -> apply_method "initialize_0" self args m >>= fun _ ->
             return self))
  | Send(e,m_name,es) ->
    ...
    (match lookup_method c_name (name_mangle m_name es) !g_class_env with
     | None -> error ("Method " ^ m_name ^ " not found")
  ...
  | IsInstanceOf(e,id) -> 
    ...
    (match obj with
      | ObjectVal(c_name, _) ->
        is_subclass c_name id !g_class_env >>= (function
          | BoolVal is -> return (BoolVal is)
          | _ -> error "IsInstanceOf: Invalid result from is_subclass")
      | _ -> error "IsInstanceOf: Invalid object")
  (* List operations* *)
  | List(es) ->
    eval_exprs es >>= fun args ->
    return @@ ListVal args
